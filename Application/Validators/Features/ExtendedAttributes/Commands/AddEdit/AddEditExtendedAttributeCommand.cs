...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    internal class AddEditExtendedAttributeCommandHandler<TId, TEntityId, TEntity, TExtendedAttribute>
        ...
        where TEntity : AuditableEntity<TEntityId>, IEntityWithExtendedAttributes<TExtendedAttribute>,
        IEntity<TEntityId>
        where TExtendedAttribute : AuditableEntityExtendedAttribute<TId, TEntityId, TEntity>, IEntity<TId>
        where TId : IEquatable<TId>
    {
        ...
        {
            if (await _unitOfWork.Repository<TExtendedAttribute>().Entities.Where(x =>
                        !x.Id.Equals(command.Id) && x.EntityId!.Equals(command.EntityId))
            ...
            else
            {
                ...
                if (extendedAttribute != null)
                {
                    ...
                    return await Result<TId>.SuccessAsync(extendedAttribute.Id,
                        "Extended Attribute Updated");
                }
                else
                {
