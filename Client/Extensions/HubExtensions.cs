...
namespace Client.Extensions
{
    ...
    {
        public static HubConnection TryInitialize(this HubConnection hubConnection, NavigationManager navigationManager, ILocalStorageService localStorage, IConfiguration config)
        {
            if (hubConnection == null)
            {
                hubConnection = new HubConnectionBuilder()
                    .WithUrl(navigationManager.ToAbsoluteUri($"{config["Urls:Api"]}{ApplicationConstants.SignalR.HubUrl}"), options => {
